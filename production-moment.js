/* Google Apps Script Web App URL — deploy apps-script/ProductionCode-Moment.gs against
   your "me'seum製作表單 - Moment款" spreadsheet, then paste the /exec URL here.
...
function setupCoverStickerUpload() {
  ...
  input.addEventListener('change', () => {
    ...
    if (count > 0 && count < MIN_COVER_FILES) {
      alert(`需上傳 ${MIN_COVER_FILES} 張照片，請重新選擇。`);
      input.value = '';
      status.textContent = '';
      return;
    }
...
function setupSubmit() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (coverCount !== MIN_COVER_FILES) {
      missing.push(`封面6張照片（需上傳 ${MIN_COVER_FILES} 張）`);
    }
